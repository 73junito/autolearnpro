defmodule LmsApi.ContentGeneration.PublishingPipeline do
  ...
  def execute_publish(pipeline_id) do
    ...
    try do
      ...
      results = Enum.reduce(pipeline.target_course_ids, %{}, fn course_id, acc ->
        course_result = publish_to_course(draft, course_id, pipeline.publish_options)
        Map.put(acc, course_id, course_result)
      end)
  ...
  defp publish_lesson(draft, course, _options) do
  ...
  defp publish_assessment(draft, course, _options) do
    content_data = draft.content_data
  ...
  defp publish_media(draft, _course, _options) do
    content_data = draft.content_data
  ...
  defp publish_interactive(draft, course, _options) do
    content_data = draft.content_data
  ...
  defp attach_media_to_lesson(_lesson_id, _media_files) do
