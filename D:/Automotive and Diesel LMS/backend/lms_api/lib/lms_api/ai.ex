defmodule LmsApi.AI do
  ...
  """
  def generate_grading_feedback(_attempt_id, student_answer, correct_answer) do
    prompt = """
  ...
  """
  def analyze_course_engagement(_course_id) do
  ...
  defp analyze_weak_areas(_progress, assessment_performance) do
  ...
  defp generate_recommendations(course, weak_topics, progress) do
    ...
    recommendations = course.modules
    |> Enum.reduce(recommendations, fn module, acc ->
      module.lessons
      |> Enum.reduce(acc, fn lesson, acc ->
    ...
    recommendations = if length(weak_topics) > 0 do
      [%{
        type: "assessment",
  ...
  defp extract_course_content(course) do
    content_parts = course.modules
    |> Enum.reduce([], fn module, acc ->
      acc = ["Module: #{module.title}" | acc]

      module.lessons
      |> Enum.reduce(acc, fn lesson, acc ->
        ["Lesson: #{lesson.title} - #{lesson.content}" | acc]
      end)
    end)
