...
Multi-page Playwright scans

- Use the bundled multi-scan runner to scan several important pages in sequence and produce per-page reports:

```bash
cd frontend/web
node tools/run_playwright_scans.js / /courses /catalog
```

This creates `a11y-home-playwright.json`, `a11y-courses-playwright.json`, etc. You can then validate each with `tools/a11y_check_exit.js` or upload them as CI artifacts.
