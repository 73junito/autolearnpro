 ...
 try {
   ...
   $localPath = 'backend/lms_api/Dockerfile'
   ...
   Write-Output "Creating blob for Dockerfile..."
   ...
   Write-Output "Creating new tree with Dockerfile at $localPath..."
   ...
   $commitBody = @{ message = "chore(ci): add backend/lms_api/Dockerfile to $targetBranch"; tree = $newTreeSha; parents = @($baseSha) } | ConvertTo-Json
   ...
   Write-Output "Done — Dockerfile added to $targetBranch. Re-dispatch the workflow now." 
} finally {
