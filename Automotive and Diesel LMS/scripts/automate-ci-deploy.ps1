...
Usage:
  .\automate-ci-deploy.ps1 -GitHubToken <PAT> [-RepoOwner 73junito] [-Repo autolearnpro] [-Workflow publish-image.yml] [-Branch k8s/env-from-health] [-ImageTag latest]
...
if (-not $GitHubToken) {
    Write-Host "No GitHub token provided — the script can still update k8s and run smoke tests if the image already exists locally or in registry."
    Write-Host "To trigger the workflow, rerun with -GitHubToken <PAT>."
}
...
kubectl -n $KubeNamespace wait --for=condition=complete job/smoke-test-idempotent --timeout=120s || Write-Warning "Smoke job didn't finish within timeout"
